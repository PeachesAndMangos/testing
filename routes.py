...
def productReview():
    if request.method == 'GET': 
        ...
        for i in user_reviews:
            if current_product_id == i.prod_id:
                return ''
        else:
            ...
            product_review= ProductReview(prod_id=current_product_id, rating=rating, comment=comments, email= current_user.email, user_id=current_user.id)
    ...
    if form.validate_on_submit():
        ...
        for i in cards:
            ...
            for i in cards:
                ...
                cardnses = CardInfo.query.filter_by(user_id=int(current_user.id))
                print(cardnses)
        ...
        if user and bcrypt.check_password_hash(user.password, form.current_password.data):
            ...
            for i in oldpassowrds:
                if bcrypt.check_password_hash(i, form.password.data):
                    print('old password')
                    flash('You cannot reuse passwords you have use before!', 'danger')
...
def deleteReview():
    ...
    if request_xhr_key and request_xhr_key == 'XMLHttpRequest':
        global current_product_id
        prod = ProductReview.query.filter_by(prod_id=current_product_id, user_id=current_user.id).first()
...
def shop():
    if current_user.is_authenticated:
        orginalCartItems = current_user.products
        id = request.args.get('id')
        itemName = request.args.get('name')
        quantity = request.args.get('quantity')
        delete = request.args.get('delete')
        product = Product.query.filter_by(prod_name=itemName, user_id = current_user.id).first() 
        if product != None and quantity != None:
            product.prod_quantity = int(quantity)
            db.session.commit()
        ...
        data = refresh()
        orginalCartItems = []
    return render_template("shop.html", data = data, cartItems = orginalCartItems, current = str(vali))

@app.route('/single_product/<int:id>')
def single_product(id):
    global current_product_id
    reviews = ProductReview.query.filter_by(prod_id=str(id))
    print(reviews)
    if current_user.is_authenticated:
        orginalCartItems = current_user.products
    ...
    with open('json_files/product.json', 'r+') as f:
        ...
        for i in data: 
            if i['id'] == id:
                current = i
    current_product_id = str(current['id'])
    prod = ProductReview.query.filter_by(prod_id=current_product_id, user_id=current_user.id).first()
    if prod == None:
        user_review = "false"
    else:
        user_review = "true"
    return render_template("single_product.html", data = current, cartItems = orginalCartItems, current2 = str(vali), reviews=reviews, user_review=user_review)
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        hashed_password = bcrypt.generate_password_hash(form.password.data).decode('utf-8')
        oldpassowrds = ast.literal_eval(user.previousPasswords)
        for i in oldpassowrds:
            if bcrypt.check_password_hash(i, form.password.data):
                print('old password')
                flash('You cannot reuse passwords you have use before!', 'danger')
        ...
        if len(oldpassowrds) == 5:
            ...
            user.previousPasswords = str(oldpassowrds)
            db.session.commit()
        else:
            oldpassowrds.append(hashed_password)
            user.password = hashed_password
